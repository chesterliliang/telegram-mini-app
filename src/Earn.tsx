...
const Profile = () => {
  ...
  const [balance] = useState<number | null>(null); // 设置为某Sol余额
  const [publicKey] = useState<string | null>(null); // 设置为公钥
  const [tokenBalance] = useState<number | null>(null); // 设置为 SPL token 余额
  const [ataAddress] = useState<string | null>(null); // 设置为ATA地址
